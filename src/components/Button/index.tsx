...
function Button({
  ...
  const buttonClassnames = clsx(
    ...
    "tw-flex tw-bg-primary tw-px-4 tw-py-2 tw-rounded-sm tw-text-white tw-items-center tw-justify-center"
  );
  ...
  const commonClassnames =
    "tw-transition-all tw-duration-200 tw-ease-out active:tw-scale-90 hover:tw-shadow-md";
  const variantClassnames = clsx(
    variant && variantClassnamesMap[variant],
    commonClassnames
  );
  return (
    <button
      ...
      {(isLoading || icon) && (
        <span
          className={clsx(
            ...
            iconPosition === PositionEnum.LEFT && "tw-mr-2",
            iconPosition === PositionEnum.RIGHT && "tw-ml-2",
            iconPosition === PositionEnum.TOP && "tw-mb-1",
            iconPosition === PositionEnum.BOTTOM && "tw-mt-1",
