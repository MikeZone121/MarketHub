...
import { BtnVariantEnum, ButtonHyperlinkProps, PositionEnum } from "./types"
...
const ButtonHyperlink = forwardRef(function Button(props: ButtonHyperlinkProps) {
  const {
    icon,
    href,
    text,
  ...
  return (
    <a href={href} className={clsx(buttonClassnames, positionClassnames, variantClassnames, className)}>
      {(isLoading || icon) && (
        <span
          className={clsx(
      ...
      <span className="tw-text-md">{text ?? children}</span>
    </a>
  )
})
